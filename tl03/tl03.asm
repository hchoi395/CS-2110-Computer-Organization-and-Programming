...
;;first, lay out stack frame
    ...
    LDR R0, R5, 4   ;R0 = node1
    LDR R0, R0, 0   ;R0 = mem[node1]
    LDR R1, R5, 5   ;R1 = node2
    LDR R1, R1, 0   ;R1 = mem[node2]

    ADD R6, R6, -1   ;push stack
    STR R0, R6, 0   ;put letter1
    JSR to_lowercase
    LDR R0, R6, 0   ;letter1 = to_lowercase(letter1)
    ADD R6, R6, 2   ;pop stack
    ADD R6, R6, -1  ;push stack
    STR R1, R6, 0   ;put letter2
    JSR to_lowercase    
    LDR R1, R6, 0   ;letter2 = to_lowercase(letter2)
    ADD R6, R6, 2   ;pop stack

    NOT R2, R1  ;R2 = -letter2
    ADD R2, R2, 1   
    ADD R2, R2, R0  ;R2 = letter1 - letter2
    BRnp ELSE_CASE
    AND R3, R3, 0   ;R3 = 0
    ADD R3, R3, 1   ;R3 = 1
    STR R3, R5, 3   ;store 1
...
;;first, lay out stack frame
    ...
    ADD R6, R6, -1  ;push stack
    LDR R1, R5, 5   ;R1 = tail
    STR R1, R6, 0   ;put tail
    ADD R6, R6, -1  ;push stack
    LDR R0, R5, 4   ;R0 = head
    STR R0, R6, 0   ;put head
    JSR are_letters_equal
    LDR R2, R6, 0   ;R2 = are_letters_equal(head, tail)
    ADD R6, R6, 3   ;pop stack
    ADD R2, R2, 0   ;access to R2
    BRnp END_IF2
    AND R3, R3, 0   ;R3 = 0
    ...
    LDR R0, R5, 4   ;R0 = head
    LDR R0, R0, 2   ;R0 = mem[head+2]
    LDR R1, R5, 5   ;R1 = tail
    LDR R1, R1, 1   ;R1 = mem[tail + 1]
    ADD R6, R6, -1  ;push stack
    STR R1, R6, 0   ;store mem[tail + 1]
    ADD R6, R6, -1  ;push stack
    STR R0, R6, 0   ;store mem[head + 2]
    JSR is_palindrome
    LDR R2, R6, 0   ;R2 = is_palindrome(new_head, new_tail)
    ADD R6, R6, 3   ;pop stack
    STR R2, R5, 3   ;return R2
