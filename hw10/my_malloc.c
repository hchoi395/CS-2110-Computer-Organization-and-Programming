...
static metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    metadata_t *newBlock = (metadata_t *)((uintptr_t) block + TOTAL_METADATA_SIZE + block->size - (TOTAL_METADATA_SIZE + size));
    newBlock->size = size;
    block->size = block->size - (TOTAL_METADATA_SIZE + size);
    return newBlock;
}
...
void *my_malloc(size_t size) {
    ...
    metadata_t *new = find_best_fit(size);
    if (!new) {
        ...
        if (temp == (void*) -1) {
            ...
            temp->size = SBRK_SIZE - TOTAL_METADATA_SIZE;
            temp->next = NULL;
            my_free((void*)((uintptr_t) temp + TOTAL_METADATA_SIZE));
            return my_malloc(size);
        }
    }
    if (new->size < size + MIN_BLOCK_SIZE) {
        remove_from_addr_list(new);
        return (void *)((uintptr_t) new + TOTAL_METADATA_SIZE);
    }
    return (void *)((uintptr_t)split_block(new, size) + TOTAL_METADATA_SIZE);
}
...
void my_free(void *ptr) {
    ...
    metadata_t *startPtr = (metadata_t*) ((uintptr_t) ptr - TOTAL_METADATA_SIZE);
    if (find_right(startPtr)) {
        metadata_t *temp = find_right(startPtr);
        remove_from_addr_list(find_right(startPtr));
        merge(startPtr, temp);
    }
    metadata_t *temp2 = find_left(startPtr);
    if (temp2) {
        remove_from_addr_list(temp2);
        merge(temp2, startPtr);
        startPtr = temp2;
    }
    add_to_addr_list(startPtr);
}
...
void *my_realloc(void *ptr, size_t size) {
    ...
    metadata_t *newPtr = my_malloc(size);
    if (!newPtr) {
        return NULL;
    }
    if (size < oldSize) {
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    metadata_t *start = my_malloc(newSize);
    if (!start) {
        return NULL;
    }
    memset(start, '\0', newSize);
    return start;
}
