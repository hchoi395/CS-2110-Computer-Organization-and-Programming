...
static int user_equal(const User *user1, const User *user2)
{
    ...
    if (user1->type == STUDENT && user2->type == STUDENT) {
        if (((!user1->name && !user2->name) || strcmp(user1->name, user2->name) == 0) && (student_equal(&user1->data.student, &user2->data.student) == 1)) {
            return 1;
        }
    }
    if (user1->type == INSTRUCTOR && user2->type == INSTRUCTOR) {
        if (((!user1->name && !user2->name) || strcmp(user1->name, user2->name) == 0) && (user1->data.instructor.salary == user2->data.instructor.salary)) {
            return 1;
        }
    }
    return 0;
...
int pop_back(LinkedList *list, User **dataOut)
{
    ...
    if (list->size == 1) {
        ...
        *dataOut = list->head->data;
        ...
        struct node *removedNode = curr->next;
...
{
    if (!list || !dataOut || list->size == 0 || index < 0 || index >= list->size) {
        return 1;
    }
    if (index == 0) {
...
int num_passing_all_classes(LinkedList *list, int *dataOut)
{
    ...
    struct node *curr = list->head;
    while (curr) {
        if (curr->data->type == STUDENT && curr->data->data.student.num_classes > 0) {
            correct = 0;
            int j = 0;
            while (j < curr->data->data.student.num_classes) {
                if ((curr->data->data.student.grades[j]) >= 60) {
                    correct++;
                }
                j++;
            }
            if (correct == curr->data->data.student.num_classes) {
                count++;
            }
        }
        curr = curr->next;
...
int get_average_salary(LinkedList *list, double *dataOut)
{
    ...
    while (curr) {
        if (curr->data->type == INSTRUCTOR) {
            count++;
            sum = sum + curr->data->data.instructor.salary; 
        }
        curr = curr->next;
    }
    if (count == 0) {
